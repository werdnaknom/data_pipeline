...
from app.UseCases.Post_Processing_UseCases.current_inrush \
    .current_inrush_results import CaptureInrushResult
from app.UseCases.Post_Processing_UseCases.process_current. \
    capture_current_results import CaptureEdgePair
from app.shared.Entities import WaveformEntity

plt.style.use("ggplot")


class InrushRequestObject(CapturePostProcessingRequestObject):
    pairs: t.List[CaptureEdgePair]
    num_axes: int
    spec_max: float
    plot_path: Path
    test_name: str = "Inrush"
    ...
    DF_SLEWRATE1: str = "meta_json_power_supply_slew_1"
    ...
    def _match_edge_pairs(self, df: pd.DataFrame) -> t.List[CaptureEdgePair]:
        ...
        # for current_testpoint in current_df.testpoint.unique():
...
class TestpointInrushUseCase(CapturePostProcessorUseCase):

    def post_process(self, request_object: InrushRequestObject) -> \
            pd.DataFrame:
        ...
        for pair in request_object.pairs:
            ...
            save_path = save_path.with_name(
                name=f"{wf_name}_{save_path.name}").with_suffix(".png")
    ...
    def capture_post_processing(self, current_wfs: t.List[WaveformEntity],
            ...
            t.Tuple[t.OrderedDict, CaptureInrushResult]:
        '''
        ...
        pair_result = CaptureInrushResult(voltage_wfs=voltage_wfs,
                                          current_wfs=current_wfs,
