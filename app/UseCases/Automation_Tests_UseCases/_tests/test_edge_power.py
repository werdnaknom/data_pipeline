...
from app.UseCases.Automation_Tests_UseCases.edge_power import \
    EdgePowerUseCase

from app.Repository.repository import Repository, MongoRepository


class TestEdgePowerUseCase(AutomationTestBaseCase):
    uc = EdgePowerUseCase
    TEST = "edge_power"
    AUTOMATION_TEST = "Aux To Main"
    DEFAULT_FILTERBY = "datacapture_id"
    PLOT_FIELDS = ["Plot"]
    EXPECTED_SHEETS = ["EdgePower"]

    def _setUp(self):
        self.uc = EdgePowerUseCase(repo=MongoRepository())
