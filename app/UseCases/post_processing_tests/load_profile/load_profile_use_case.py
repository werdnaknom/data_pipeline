...
from app.UseCases.post_processing_tests.post_processing_usecase import PostProcessingWaveFormUseCase
...
class LoadProfileUseCase(PostProcessingWaveFormUseCase):
    by: t.List[str] = ["dut"]
    ...
    def process_request(self,
        ...
        The Load Profile test has to do 3 main functions for each capture:
            ...
            - Validate waveforms are stable/within spec during the entire test
        '''
        ...
        for i, group_dict in enumerate(capture_groups):
            ...
            validated_current_results = self.validate_current(
                waveforms=group_wfs, df=group_df)
            '''
